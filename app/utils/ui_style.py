...
def apply_login_form_style():
    """Applique le style du formulaire ET le style pro de la navigation sur la page de login."""
    st.markdown(
        """
        <style>
        /* 🌟 AJOUT PRÉCIS : Forcer uniquement le texte d'invitation hors du formulaire en noir */
        [data-testid="stMarkdownContainer"] p:not(.stForm p):not(.stForm label) {
            color: #000000 !important;
        }

        /* --- STYLE DES PAGES DE NAVIGATION (Copie conforme pour le login) --- */
        [data-testid="stSidebarNav"] > ul > li > div > a,
        [data-testid="stSidebarNav"] > ul > li > div > span > a {
            background-color: #00C8C8 !important;
            color: white !important;
            ...
            margin-left: auto !important;
            margin-right: auto !important;
            pointer-events: none !important; /* 🔒 Sécurité : Empêche de cliquer sur les pages au login */
            opacity: 0.8; /* Légèrement estompé pour montrer que c'est verrouillé */
        }

        /* Icône avec puce blanche par défaut pour toutes les pages au login */
        [data-testid="stSidebarNav"] > ul > li > div > a::before,
        [data-testid="stSidebarNav"] > ul > li > div > span > a::before {
            content: "•";
            margin-right: 5px;
        ...
        /* --- Ton code existant pour le formulaire --- */
        .stForm {
        ...
        .stForm .stTextInput > label {
            color: white !important; /* On force le blanc pour les noms des champs */
        }
        </style>
        """,
        unsafe_allow_html=True
